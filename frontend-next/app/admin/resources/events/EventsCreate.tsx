        ...
        <Box sx={{ mb: 2 }}>
            ...
            <Editor
                ...
                init={{
                    ...
                    images_upload_handler: createTinyMCEUploadHandler('events'),
...
// Компонент для условного отображения поля ссылки на мероприятие
const EventLinkInput = () => {
    const { watch } = useFormContext();
    const ours = watch('ours');
    ...
    return (
        <TextInput
            source="eventLink"
            label="Ссылка на мероприятие (трансляция)"
            helperText="Заполняется только для наших мероприятий"
            fullWidth
        />
    );
};

const EventsCreateActions = () => (
    <TopToolbar>
        <ListButton />
    </TopToolbar>
);

const EventsCreateToolbar = () => {
    ...
    const handleSave = handleSubmit(async (data) => {
        try {
            ...
            const { tempImages: _, ...eventData } = data;

            await create('events', { data: eventData });
            notify('Мероприятие успешно создано');
            redirect('/admin#/events');
        } catch (error) {
            console.error('Error creating event:', error);
            notify('Ошибка при создании мероприятия', { type: 'error' });
        }
    });
...
export const EventsCreate = () => (
    <Create title="Создание мероприятия" actions={<EventsCreateActions />}>
        <Card className="p-6">
            <SimpleForm toolbar={<EventsCreateToolbar />}>
                <TextInput
                    source="title"

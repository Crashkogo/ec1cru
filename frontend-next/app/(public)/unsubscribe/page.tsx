    ...
    const unsubscribe = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/posts/unsubscribe?token=${token}`
        );
        ...
        if (
          ...
          error.response?.data?.message?.includes('Already unsubscribed')
        ) {
          setStatus('already');
      ...
      <div className="max-w-md w-full">
        <Card className="!shadow-lg !border-0 !rounded-2xl p-8">
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-modern-gray-900 mb-3">{getTitle()}</h1>

            <p className="text-modern-gray-600 mb-6">{message}</p>

            <p className="text-sm text-modern-gray-500 mb-6">{getDescription()}</p>
            ...
            {status === 'error' && (
              <div className="bg-red-50 rounded-lg p-4 mb-6">
                <h3 className="text-sm font-medium text-red-900 mb-2">Нужна помощь?</h3>
                <p className="text-sm text-red-700">Свяжитесь с нами:</p>
                <div className="mt-2">
                  <a
    ...
    <Suspense
      fallback={
        <div className="min-h-screen bg-modern-gray-50 flex items-center justify-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-modern-primary-600"></div>
        </div>
      }
    >
      <UnsubscribeContent />

...
const CompanyLife: React.FC = () => {
  ...
  const fetchPosts = async (reset = false) => {
    ...
    try {
      const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/posts/company-life`, {
        params: {
          page: reset ? 1 : page,
          limit: itemsPerPage,
  ...
  return (
    <>
      <Helmet>
        <title>Жизнь компании - ООО «Инженер-центр»</title>
        <meta name="description" content="Новости и события из жизни компании «Инженер-центр» - наши достижения, мероприятия и интересные моменты" />
      </Helmet>
                ...
                {posts.length === 0 && !loading ? (
                  <div className="text-center py-12 bg-white rounded-xl shadow-modern">
                    ...
                    <p className="text-modern-gray-600 mb-4">
                      {searchQuery || dateFrom || dateTo ? 'Попробуйте изменить параметры поиска' : 'Посты скоро появятся!'}
                    </p>
                    {(searchQuery || dateFrom || dateTo) && (
                      <button
                        onClick={resetFilters}

...
const Promotions: React.FC = () => {
    ...
    // Загрузка акций
    const fetchPromotions = async (reset = false) => {
        if (loading || (!hasMore && !reset)) return;
        setLoading(true);

        try {
            const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/posts/promotions`, {
                params: {
                    page: reset ? 1 : page,
                    limit: itemsPerPage,
            ...
            setPromotions((prev) => (reset ? newItems : [...prev, ...newItems]));
        ...
        <>
            ...
            <div className="mb-6">
                ...
                <div className="relative">
                    ...
                    <input
                        ...
                        autoComplete="off"
                        className="w-full pl-10 pr-4 py-3 border border-modern-gray-300 rounded-lg focus:ring-2 focus:ring-modern-accent-500 focus:border-modern-accent-500 transition-all duration-200 text-modern-gray-900 placeholder-modern-gray-500"
                    />
                </div>
            </div>
                ...
                <div className="space-y-2">
                    <label className="flex items-center">
                        <input
                            ...
                            className="mr-2 text-modern-accent-600 focus:ring-modern-accent-500"
                        />
                        <span className="text-sm text-modern-gray-700">Все акции</span>
                    </label>
                    <label className="flex items-center">
                        <input
                            type="radio"
                            ...
                            className="mr-2 text-modern-accent-600 focus:ring-modern-accent-500"
                        />
                        <span className="text-sm text-modern-gray-700">Активные</span>
                    </label>
                    <label className="flex items-center">
                        <input
                            type="radio"
                            ...
                            className="mr-2 text-modern-accent-600 focus:ring-modern-accent-500"
                        />
                        <span className="text-sm text-modern-gray-700">Завершённые</span>
                    </label>
                </div>
            </div>
            ...
            <div className="mb-6">
                ...
                <input
                    ...
                    className="w-full px-3 py-2 border border-modern-gray-300 rounded-lg focus:ring-2 focus:ring-modern-accent-500 focus:border-modern-accent-500 text-modern-gray-900"
                />
            </div>
    ...
    ), [searchQuery, handleSearchChange, statusFilter, dateFrom, handleDateFromChange, dateTo, handleDateToChange, resetFilters]);
            ...
            <>
                ...
                <div className="min-h-screen bg-modern-gray-50 flex items-center justify-center">
                    <div className="text-center">
                        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-modern-accent-600 mx-auto mb-4"></div>
                        <p className="text-modern-gray-600">Загрузка акций...</p>
                    </div>
                </div>
            </>
        );
            ...
            <div className="min-h-screen bg-modern-gray-50">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                    ...
                    <div className="lg:hidden mb-4">
                        ...
                        >
                            <AdjustmentsHorizontalIcon className="h-5 w-5 mr-2 text-modern-accent-600" />
                            <span className="font-medium text-modern-gray-900">Фильтры</span>
                            {(searchQuery || statusFilter !== null || dateFrom || dateTo) && (
                                <span className="ml-2 px-2 py-0.5 bg-modern-accent-100 text-modern-accent-700 rounded-full text-xs">
                                    Активны
                                </span>
                            )}
                        </button>
                        ...
                        <div className="flex-1 lg:w-5/6">
                                        ...
                                        <div className="flex items-start justify-between">
                                            ...
                                            <div className="flex-1 mr-6">
                                                <div className="flex items-center mb-2">
                                                    <div className="p-2 bg-modern-accent-100 rounded-lg mr-3">
                                                        <TagIcon className="h-5 w-5 text-modern-accent-600" />
                                                    </div>
                                                    <h3 className="text-xl font-semibold text-modern-gray-900 group-hover:text-modern-accent-600 transition-colors duration-200">
                                                        {item.title}
                                                    </h3>
                                                    <span className={`ml-3 px-2 py-1 rounded-full text-xs font-medium ${item.status
                                                        ? 'bg-green-100 text-green-700'
                                                        : 'bg-modern-gray-100 text-modern-gray-600'
                                                        }`}>
                                                        {item.status ? 'Активна' : 'Завершена'}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex items-center space-x-4 text-sm text-modern-gray-500">
                                                    ...
                                                    <div className="flex items-center">
                                                        ...
                                                        <span>До {new Date(item.endDate).toLocaleDateString('ru-RU')}</span>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {/* Стрелка */}
                                            <div className="flex items-center">
                                                <ArrowRightIcon className="h-5 w-5 text-modern-gray-400 group-hover:text-modern-accent-600 group-hover:translate-x-1 transition-all duration-200" />
                                            </div>
                                        </div>
                                    </Link>
                                ))}
                            ...
                            {loading && (
                                <div className="text-center mt-8">
                                    <div className="inline-block w-6 h-6 border-2 border-modern-accent-600 border-t-transparent rounded-full animate-spin"></div>
                                    <p className="text-modern-gray-600 mt-2 text-sm">Загружаем ещё акции...</p>
                                </div>
                            )}
                            ...
                            {!loading && promotions.length === 0 && (
                                <div className="text-center py-16">
                                    <TagIcon className="h-16 w-16 text-modern-gray-300 mx-auto mb-4" />

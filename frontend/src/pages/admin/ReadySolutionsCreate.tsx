// src/pages/admin/ReadySolutionsCreate.tsx
...
const ReadySolutionsCreate: React.FC = () => {
  ...
  // Обновим handleGalleryImageUpload
  const handleGalleryImageUpload = async (_e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    <div className="mt-2 flex space-x-2">
      {galleryImages.map((img, idx) => (
        <img
          ...
          onError={() => console.error('Image load error:', img)} // Логируем ошибки загрузки
        />
      ))}
    </div>
  )}
  ...
  const onSubmit: SubmitHandler<SolutionFormInputs> = async (data) => {
    ...
    try {
      ...
      const solutionData = {
        ...
        images: movedGalleryImages,
        programIds: selectedPrograms,
      };

      await axios.post(`${import.meta.env.VITE_API_URL}/api/posts/admin/ready-solutions`, solutionData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      alert('Ошибка при создании решения: ' + (error instanceof Error ? error.message : 'Неизвестная ошибка'));
    }
  };

  return (
    <div className="w-full mx-auto bg-white p-6 rounded-lg shadow-md">
      <h2 className="text-xl font-bold mb-4">Создание готового решения</h2>
